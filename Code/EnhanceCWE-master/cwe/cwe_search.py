...
class CWESearchLocator:
    ...
    def register(cwe_search):
        ...
        if isinstance(cwe_search, CWESearchBase):
            ...
            if cwe_search.get_algorithm_name() in CWESearchLocator.service_list:
                raise ValueError('This search algorithm has already been registered!')

            if not CWESearchLocator.service_provider:
                CWESearchLocator.service_provider = cwe_search

            CWESearchLocator.service_list[cwe_search.get_algorithm_name()] = cwe_search
    ...
    def get_instance():
        ...
        default_instance = CWESearchLocator.service_provider
        if (default_instance == None):
        ...
        if SearchLocator.objects.exists():
            locator = SearchLocator.objects.all()[0]
            if locator.name != "Default" and locator.name in CWESearchLocator.service_list:
                return CWESearchLocator.service_list[locator.name]
...
class CWESearchBase:
    ...
    def get_algorithm_name(self):
        """
        This gives a description name of this search algorithm
...
class CWEKeywordSearch(CWESearchBase):
    ...
    def get_algorithm_name(self):
        return "Basic Search - Stopwords & Stem"
...
class TestSearch(CWESearchBase):
    ...
    def get_algorithm_name(self):
        return "Test Search - Same as Basic"
...
# The first CWESeachAlgo registered would be the default search algorithm
...
CWESearchLocator.register(CWEKeywordSearch())
CWESearchLocator.register(TestSearch())
